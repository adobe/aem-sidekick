...
export class LoginButton extends LitElement {
  ...
  renderLogin() {
    ...
    if (appStore.state === SidekickState.LOGIN_REQUIRED) {
      return html`<sp-action-button quiet class="login" @click=${this.login}>${appStore.i18n('user_login')}</sp-action-button>`;
