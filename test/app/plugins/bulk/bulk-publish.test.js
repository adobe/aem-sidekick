...
describe('Bulk preview plugin', () => {
  ...
  beforeEach(async () => {
    ...
    publishStub = sidekickTest.sandbox.stub(bulkStore, 'publish');
  });
  ...
  it('bulk publish calls bulkStore.publish()', async () => {
    ...
    const bulkPreviewPlugin = recursiveQuery(sidekick, '.bulk-publish');
    expect(bulkPreviewPlugin.textContent.trim()).to.equal('Publish');
