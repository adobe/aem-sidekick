...
describe('Plugin action bar', () => {
  ...
  describe('test repositioning', () => {
    ...
    it('ends dragging when mouse leaves viewport', () => {
      // Start drag
      actionBar.onDragStart(new MouseEvent('mousedown', {
        bubbles: true,
        cancelable: true,
    ...
    it('removes custom position on resize below 800px after throttle', async () => {
      // Set custom position
      actionBar.style.left = '50%';
    ...
    it('constrains position on resize above 800px after throttle', async () => {
      // Set position beyond viewport
      actionBar.style.left = '50%';
      actionBar.style.transform = 'translate(-2000px, 0px)';
      actionBar.style.bottom = '50px';

      // Mock window.innerWidth to be above 800px
      const originalInnerWidth = window.innerWidth;
      Object.defineProperty(window, 'innerWidth', {
        writable: true,
        configurable: true,
      ...
      // Spy on constrainToViewport
      const constrainSpy = sidekickTest.sandbox.spy(actionBar, 'constrainToViewport');
      ...
      // constrainToViewport should have been called
      expect(constrainSpy.calledOnce).to.be.true;
    ...
    it('does not end drag on mouse leave when not dragging', () => {
      expect(actionBar.isDragging).to.be.false;

      const onDragEndSpy = sidekickTest.sandbox.spy(actionBar, 'onDragEnd');

      // Call onMouseLeave when not dragging
      actionBar.onMouseLeave();

      // onDragEnd should NOT be called
      expect(onDragEndSpy.called).to.be.false;
    });
